...
char* get_cookie_list(request_info* request, char* item){
	char* item_cookies[] = {"item1","item2","item3","item4","item5","item6","item7","item8","item9","item10","item11","item12"};	
	...
	for(i=1; i<total_items; i++){
		...
		curr_cookie = extract_cookie(request->cookie, item_cookies[i-1]);
...
void handle_addcart(request_info* request, response_info* response){
	...
	if (item_num && item){
		...
		response->body = get_cookie_list(request, item);
	}
}
